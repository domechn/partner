...
let mainWindow = null;
let latestGaze = { x: 0.5, y: 0.5 };
let pendingConfirmation = null;
const isDev = !!process.env.VITE_DEV_SERVER_URL;
const riskyActions = new Set(['open_app']);
...
ipcMain.handle('gaze:update', (_event, point) => {
    latestGaze = {
        x: Math.min(1, Math.max(0, point.x)),
        y: Math.min(1, Math.max(0, point.y)),
    };
});
ipcMain.handle('automation:request', async (_event, request) => {
    ...
    try {
        if (request.kind === 'confirm_pending') {
            ...
            const toRun = { ...pendingConfirmation, confirmed: true };
...
async function runAutomation(activeWindow, request) {
    switch (request.kind) {
        case 'click_here': {
